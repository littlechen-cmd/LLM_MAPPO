...
NARROW_PATH = LAYOUT_DIRECTORY / "unseen_narrow_passage_v2.txt"
CENTRAL_PATH = LAYOUT_DIRECTORY / "unseen_central_cross_v2.txt"

NARROW_CORRIDOR = tuple((10, y) for y in range(7, 16))
NARROW_CHARGING_STATIONS = (
    (19, 2), (16, 5), (6, 2), (0, 5),
    (19, 11), (16, 18), (6, 18), (0, 21),
)
CENTRAL_CENTER = (10, 11)
CENTRAL_ARMS = {
    "north": ((10, 8), (10, 9), (10, 10)),
    "south": ((10, 12), (10, 13), (10, 14)),
    "west": ((7, 11), (8, 11), (9, 11)),
    "east": ((11, 11), (12, 11), (13, 11)),
}
...
def _read_layout(path):
    ...
    assert len(rows) == 24
    assert {len(row) for row in rows} == {20}
...
def test_narrow_map_has_one_eight_cell_loaded_transport_corridor():
    ...
    assert goals == {(5, 0), (14, 0)}
    assert set(NARROW_CORRIDOR) <= highways
    assert not (set(NARROW_CORRIDOR) & set(NARROW_CHARGING_STATIONS))
    assert any(
        rows[y][x] == "X" for y in range(0, 7) for x in range(20)
    )
    assert any(
        rows[y][x] == "X" for y in range(16, 24) for x in range(20)
    )
    for articulation in NARROW_CORRIDOR[1:-1]:
        ...
        assert any((19, 23) in item for item in components)
...
def test_central_map_has_one_four_way_loaded_transport_articulation():
    ...
    endpoints = {(10, 8), (10, 14), (7, 11), (13, 11)}
    assert {
