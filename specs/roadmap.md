...
- 最低门槛：有效教师覆盖率≥25%；无 NaN/Inf 或接口失败；相对 `MAPPO-WP` 的三 seed 标准化
  吞吐 AUC 中位数退化≤10%。
...
- 优化路线预算：核心 `2×2`、QMIX-WP 与 RuleKD 各 8 seed；ShuffleKD 与 NoWP 各 3 诊断
  seed；启发式 A* 无训练；两个真正未见拓扑按后续冻结合同统一评估；不做 8-AGV 压力实验。
...
- [ ] pending
- 目标：从冻结原始日志生成统计汇总、置信区间、效应量、表格、图像和失败案例；完成方法—
  实验—论文一致性审计。
