...
O3 的规格、地图冻结、确定性、接口和防泄漏验证可在 O1 等待 owner-only A600 门禁时并行。
O1 未通过前 O2 仍被硬阻塞；O1、O2、O3 与 S1 只在 D1 汇合，不得互相替代验收证据。
...
- [~] in progress
- 状态：本地实现、短 smoke 与静态验证已完成，等待研究所有者运行 A600 runtime/memory gate；
  设备占用不会降低、跳过或改写该门禁。
...
- 验收：只构成拓扑与评估协议就绪门；确定性环境测试、安全测试、接口 smoke、地图哈希和
  防泄漏审计通过。O3 不加载学习策略、不产生完成率或难度结论；真正的未见拓扑性能评估仅在
  D1 选定优化路线后于 E2 运行。
...
- [ ] pending
- 优化路线选择条件：O0 人工批准、O1 本地验证与 owner-run A600 runtime/memory 门通过、O2
  校准门通过、O3 拓扑/接口就绪门通过。
...
- 研究所有者在 A600 上运行全部长任务；Codex 和项目工程师分别准备所负责路线的命令，并对
  产物进行独立分析和交叉审查。
- 正式评估固定使用 held-out evaluation seeds `200–209 × 20 episodes`，统一 final checkpoint；训练 seed 是
  学习方法比较的独立统计单位。
