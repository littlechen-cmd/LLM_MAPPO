...
- [~] O3-A 至 O3-F 已实现并通过本地验证；等待 owner 确认“拓扑/接口就绪”
- 状态：本地实现、短 smoke 与静态验证已完成；需按 2026-08-27 资源修订将常规门禁改为
  `baseline/H12`，H4 仅在失败后诊断。研究所有者随后以一个 A600 作业先运行 O1 门禁，通过后
  才自动进入 O2；设备占用不会降低或跳过该门禁。
...
- [ ] pending
- 目标：由研究所有者在同一 A600 作业中先通过 O1 门禁，再运行匹配校准训练；O1/O2 产物与
